...
# Data output: a large number of randomisations of the original dataset, and plot of 
...
for (i in 1:length(list.CCF.means)) {
  ...
  lines(lag.in.s, mean.ccf.real.top.cluster,
      col = "#225EA8",
      lwd = 3)
  legend("topright", legend = "Observed", text.col = "#225EA8", bty = "n")
...
#scale.edges <- c(-0.6, 1)
scale.breaks <- seq(scale.edges[1], scale.edges[2], length.out = 10)
